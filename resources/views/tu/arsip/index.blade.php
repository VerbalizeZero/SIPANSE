<x-app-layout>
    {{-- Halaman utama Arsip untuk role TU. --}}
    <div class="py-8">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mb-4">
                <h1 class="text-2xl font-semibold text-slate-900">Arsip</h1>
                <p class="text-sm text-slate-500">Kumpulan riwayat faktur yang telah selesai dan diarsipkan</p>
            </div>

            {{-- Baris filter + tombol Global Export --}}
            <div class="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <form method="GET" action="{{ route('tu.arsip.index') }}" class="flex flex-1 flex-col gap-2 md:flex-row md:items-center">
                    <input
                        type="month"
                        name="bulan"
                        value="{{ $filters['bulan'] ?? '' }}"
                ...
                <a
                    href="{{ route('tu.arsip.export_global', ['bulan' => $filters['bulan'] ?? '', 'kelas' => $filters['kelas'] ?? '', 'search' => $filters['search'] ?? '']) }}"
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white text-sm text-slate-700">
                            @forelse ($fakturs as $faktur)
                                @php
                                    ...
                                    // Semua data di sini berwarna hijau Emerald karena berstatus Arsip
                                @endphp
                                <tr class="hover:bg-slate-50 transition">
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <form method="POST" action="{{ route('tu.arsip.export_sublist', $faktur) }}" class="inline-block">
                                            @csrf
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-12 text-center">
                                        ...
                                        <p class="mt-4 text-sm text-slate-500 font-medium">Buku Arsip masih kosong.</p>
                                        <p class="mt-1 text-xs text-slate-400">Belum ada faktur yang selesai diverifikasi dan mencapai tahap arsip.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
